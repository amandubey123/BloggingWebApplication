...
const [file,setFile]= useState()
  ...
  const formSchema = z.object({
    name: z.string().min(3, "Name must be three character long."),
    email: z.string().email(),
    bio: z.string().min(3, "Bio must be three character long."),
  ...
  return (
    <Card className="max-w-screen-md mx-auto bg-gray-600 text-white">
      <CardContent>
            ...
            {({ getRootProps, getInputProps }) => (
              <div {...getRootProps()}>
                ...
                <Avatar className="w-28 h-28 relative group">
                  <AvatarImage src={filePreview? filePreview: userData?.user?.avatar || user1} />
                  <div
                    className="absolute z-50 w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
            justifiy-center items-center bg-gray-600 bg-opacity-20 border-2 border-white rounded-full
            group-hover:flex hidden cursor-pointer"
                  >
                    <IoCameraOutline className="ml-3 size-20" />
                  </div>
                </Avatar>
              </div>
            )}
